...
- **Explicit writes only**: reviewr mutates Git only for the user's `C` commit, confirmed `D` discard, or `P` push in uncommitted Changes. Its automatic Git writes are private refs under `refs/reviewr/`.
...
The runtime is a single-threaded frame loop (`event_loop` in `src/lib.rs`): draw → wait for input or poll deadline → mutate `App` → draw. Clipboard, agent-send, and per-file diff builds run synchronously between frames. Worker threads own the world refresh and turn tracking, search, PR input probe, PR forge fetch, config recovery, and serialized explicit repository actions. World results land through `land_world_completion`: input-tagged, latest-wins, reconciled only while the view still matches (see Continuity above).
...
- `src/git.rs` — read-side Git subprocesses plus reviewr's private refs. `changed_files` (scope changesets), `all_files` (tracked + untracked + ignored via `ls-files` — never use `git status --ignored`, it walks inside ignored trees and costs seconds), `snapshot_worktree` (temp-index `add -A` + `write-tree` for turn baselines), baseline refs.
- `src/repo_actions.rs` — the single explicit mutation boundary: guarded commit/discard and configured-upstream push, serialized on its own worker.
